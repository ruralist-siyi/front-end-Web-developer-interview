...
let currentEventTime: ExpirationTime = NoWork;
 ...
 // 这里的now 是通过window.performance.now()当前时间的毫秒戳
export function requestCurrentTimeForUpdate() {
  // 判断当前 executionContext 是否处在 RenderContext 或者是 CommitContext 的阶段。
  // RenderContext代表着React正在计算更新，CommitContext代表着React正在提交更新。
...
// NoMode = 0b0000; BlockingMode = 0b0010; ConcurrentMode = 0b0100;
// RenderContext = b010000; NoContext = 0b000000;
export function computeExpirationForFiber(
  currentTime: ExpirationTime,
  fiber: Fiber, // HostRootFiber
  suspenseConfig: null | SuspenseConfig,
): ExpirationTime {
  const mode = fiber.mode; // 这是在createHostRootFiber中我们根据传入tag ConrrentMode or BlockingRoot判断的
  if ((mode & BlockingMode) === NoMode) {
    return Sync; // 如果fiber.mode 等于 BlockingMode 就返回Sync
  }
  // 获取当前的优先等级
  const priorityLevel = getCurrentPriorityLevel();
  if ((mode & ConcurrentMode) === NoMode) {
     // 如果fiber.mode 等于 ConcurrentMode ：在根据 priorityLevel 来判断是 Sync 还是Batch
    return priorityLevel === ImmediatePriority ? Sync : Batched;
  }
  // 如果 当前的 executionContext 是 RenderContext， 也就是处于Render阶段
  ...
  let expirationTime;
  if (suspenseConfig !== null) { // 如果有suspenseConfig 就根据 suspenseConfig 去计算expirationTime
    ...
    // 根据Schedule 优先级来计算expirationTime
    switch (priorityLevel) {
      ...
      case LowPriority: // TODO: Handle LowPriority
        ...
        // 计算低优先级update的expirationTime（普通的async update）
  ...
  // 渲染进行中时 workInProgressRoot 指向 FiberRoot，如果计算出的 expirationTime 与 renderExpirationTime相同，
  // 我们就降低此次更新的优先级
...
export function scheduleUpdateOnFiber(
  ...
  // 获取我们的FiberRoot对象
  /**
   * by: 司徒（ReactFiber在并发模式下的运行机制）
   * 将当前fiber及其alternate的过期时间推迟（或者叫加大）。由于在ReactFiber中，过期时间等价于优先级，
   * 换言之，一个组件在某个时间段setState频繁，那么它就越优先更新。
   */
  ...
  // 检查是否当前的任务是否被打断，如果是，就用 interruptedBy保存传入的fiber对象
  ...
  if (expirationTime === Sync) {
    ...
    if (
      ...
      // 为每一个update任务进行优先级排序，并且可以记录调度到了哪里
      ...
      // 不经过schedule
      /**
       * https://zhuanlan.zhihu.com/p/108849110
       * 1. 一次更新的产生，并不只影响当前 Fibre 对象，而是会影响到所有上层的 Fiber 节点，
       * 修改了当前 Fiber 对象的 expirationTime，修改了 Fiber 父节点 的 childExpirationTime。
       * 2. 首次渲染，React 不会做任何调度，直接 renderRoot，因为首次渲染是没有调度的必要的。
       */
...
function performSyncWorkOnRoot(root) {
  ...
  if (root.finishedExpirationTime === expirationTime) {
    ...
    if (
      ...
      // 创建一个WIP(Working In Progress)节点
      prepareFreshStack(root, expirationTime);
      startWorkOnPendingInteractions(root, expirationTime);
    }
...
export function unbatchedUpdates<A, R>(fn: (a: A) => R, a: A): R {
  // 这是上面的代码复制下来参考理解（这些Context常量对应React中的执行栈的一个标识， executionContext代表当前的执行栈）
  // Describes where we are in the React execution stack
  // let executionContext: ExecutionContext = NoContext;
  ...
  // const LegacyUnbatchedContext = /*       */ 0b001000; 2的3次方 = 8
  // const RenderContext = /*                */ 0b010000; 2的4次方 = 16
  // const CommitContext = /*                */ 0b100000; 2的5次方 = 32

  // 保存当前的执行栈
  const prevExecutionContext = executionContext;
  // 改变现在的executionContext（此段就是将executionContext改变为LegacyUnbatchedContext，非批量更新模式）
  // executionContext = executionContext & (~BatchedContext)
  executionContext &= ~BatchedContext; // 0b0000000 = 0b000000 & 0b111110 (结果算出来也就是executionContext在这个语句中还是等于NoContext)
  // executionContext = executionContext | LegacyUnbatchedContext
  executionContext |= LegacyUnbatchedContext; // 0b001000 = 0b0000000 | 0b001000(算出来也就是将当前的executionContext 变为 LegacyUnbatchedContext)
  try {
    // 回调也就是我们的updateContainer
    return fn(a);
  } finally {
    // 最后又把之前暂存的执行栈恢复
    executionContext = prevExecutionContext;
...
function workLoopSync() {
  ...
  while (workInProgress !== null) {
    // performUnitOfWork返回一个指向workInProgress tree树上某个还有未完成work的fibe node的引用
    // 一旦是null的话，React就会退出work loop，然后准备进入commit阶段。

    // 核心也就是深度优先遍历：先处理完所有children的work再回溯到parent节点
...
/**
 * 好文推荐： 作者：鲨叔 链接：https://juejin.im/post/5e92e592f265da48027a2c5d
 */
function performUnitOfWork(unitOfWork: Fiber): Fiber | null {
  ...
  /**
   * beginWork 方法会返回下一个要执行的任务（next），
   * 如果 next 为空表示已经遍历到叶子节点了，则调用 completeUnitOfWork 可以执行完成逻辑
   */
  let next;
  if (enableProfilerTimer && (unitOfWork.mode & ProfileMode) !== NoMode) {
    ...
    // beginWork 函数要么返回一个指向下一个work loop需要处理的fiber node的引用，要么返回null。
    next = beginWork(current, unitOfWork, renderExpirationTime);
  }
  ...
  if (next === null) {
    ...
    // 一旦一个fiber node被结束掉，React接着会执行它的sibling节点的work，
    // 在完成这个sibling的这个分支后，就会移步到下一个sibling节点.....以此类推。
    // 当所有的sibling节点被结束到，React才会回溯到parent节点
...
/**
 * 实现主体是一个大while循环
 * 当workInProgress node没有children的时候，React的执行就会进入这个函数。
 * 在完成当前fiber node的work之后，它就紧接着去检查是否还有下一个sibling节点要处理。
 * 如果有的话，那么就把下一个sibling的引用返回出去，退出当前函数。
 * 所返回的sibling的引用会赋值给nextUnitOfWork（workInProgress）变量，然后React就从这个sibling开始新一轮的遍历。
 */
  ...
  do {
    ...
    if ((workInProgress.effectTag & Incomplete) === NoEffect) {
      ...
      if (
        ...
        // side-effect order.
        // 这里很重要，我们的effect list就是在这里处理出来的
        // 父级fiber的firstEffect属性指向当前的fiberNode的workInProgress上的firstEffect
...
function flushPassiveEffectsImpl() {
  ...
  while (effect !== null) {
    if (__DEV__) {
      ...
      try {
        // passive effects 和 effect hooks相关
