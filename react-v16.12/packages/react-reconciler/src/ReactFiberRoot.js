...
/**
 * 每一个DOM容易都会创建一个FiberRootNode
 * 可以通过此方法访问：const fiberRoot = document.querySelector('#container')._reactRootContainer._internalRoot
 */
function FiberRootNode(containerInfo, tag, hydrate) {
  this.tag = tag; // 对应的tag LegcyRoot等
  this.current = null; // 也就是后面赋值的HostRootFiberNode FiberTree的根节点
  this.containerInfo = containerInfo; // containerInfo就是我们传入的dom容器节点
  this.pendingChildren = null;
  this.pingCache = null;
  this.finishedExpirationTime = NoWork;
  this.finishedWork = null; // commit阶段的时候就是去操作 finishedWork的属性值
  this.timeoutHandle = noTimeout;
  this.context = null;
  this.pendingContext = null;
  this.hydrate = hydrate; // 服务端渲染 hydrate
