...
// 除以UNIT_SIZE（10） 原因是可以抹平10ms以内的误差
...
// 计算我们各种优先级的expirationTime
// MAGIC_NUMBER_OFFSET = （Max 31 bit integer. 系统整数最大值 ）- 2
// UNIT_SIZE 常量 10
// 结果就是：低优先的过期时间间隔是25ms 高优先级的过期时间间隔是10ms
/**
 * React低优先级update的expirationTime间隔是25ms，
 * React让两个相近（25ms内）的update得到相同的expirationTime，
 * 目的就是让这两个update自动合并成一个Update，从而达到批量更新的目的，
 * 就像LOW_PRIORITY_BATCH_SIZE的名字一样，自动合并批量更新。
 * 链接：https://juejin.im/post/5d6a572ce51d4561fa2ec0bc
 * @param {*} currentTime
