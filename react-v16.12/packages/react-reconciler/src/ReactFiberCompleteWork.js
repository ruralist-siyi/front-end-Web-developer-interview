...
if (supportsMutation) {
  ...
  updateHostComponent = function(
    ...
    // related to `hidden`.
    // prepareUpdate 方法核心就是
    // 得到我们更新的内容： const updatePayload = diff(oldProps, newProps, viewConfig.validAttributes);
    ...
    // 把需要更新的内容放在workInProgress.updateQueue 上，这是个array
...
function completeWork(
  ...
  switch (workInProgress.tag) {
    ...
    case HostComponent: {
      ...
      // 判断是第一次mount还是update 以及 对应的Fiber节点的dom节点是否存在
      // 如果dom节点已经存在那么我们只需要考虑更新状态就行了，不需要重新生成dom元素
      if (current !== null && workInProgress.stateNode != null) {
        ...
        if (wasHydrated) {
          ...
          // 通过while循环通过workInProgress.child讲instance的子dom都挂到instance上
          /**
           * 当你第一次render的时候 dom树是如何构建出来的呢？
           * 每次深度优先遍历，就会将我们每个Fiber Node对应的dom生成以及子dom插入到此dom上
           * 不断的归并到rootFiber我们就完整的拥有了一颗“离屏dom树”
           */
          appendAllChildren(instance, workInProgress, false, false);

          // This needs to be set before we mount Flare event listeners
          // 将对应的dom结构挂到workInProgress.stateNode上
